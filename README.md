...
## 📁 V. Cấu trúc Mã nguồn & Triển khai (Project Structure)

Dự án được chia thư mục theo chuẩn mô hình CI/CD của cơ sở dữ liệu, đảm bảo dễ dàng chạy kịch bản (script) theo đúng thứ tự:

```text
📁 G09_CinemaCore
 ├── 📁 01_DDL_Tables          -- Khởi tạo cấu trúc bảng vật lý và Ràng buộc (Constraints)
 ├── 📁 02_DML_MockData        -- Dữ liệu mẫu (Seed Data) mô phỏng môi trường thực tế
 ├── 📁 03_Functionalities     -- Kiến trúc logic nghiệp vụ (Views, Functions, Triggers)
 ├── 📁 Data_Generator         -- Script Python sinh dữ liệu tự động (Faker)
